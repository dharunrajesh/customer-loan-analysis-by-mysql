...
create table loan_status (loan_id varchar (30) , customer_id varchar (30) , loan_amount varchar(30) ,loan_amount_term int , cibil_score int, primary key (loan_id));

create table remarks_update (loan_id varchar (30), loan_amount varchar (40), cibil_score int, cibil_score_status varchar(100), primary key (loan_id));

show tables;

-- trigger 1
delimiter $$
create trigger remarks after insert on loan_status for each row
begin
if new.cibil_score >900 then
insert into remarks_update (loan_id	,loan_amount,Cibil_Score,Cibil_Score_status) values
...
insert into remarks_update (loan_id	,loan_amount,Cibil_Score,Cibil_Score_status) values
(new.loan_id, new.Loan_amount,new.Cibil_Score, 'reject customers(loan cannot apply)');
...
-- trigger2
delimiter //
create trigger remarks_det before insert on loan_status for each row
begin
...
-- inserting values into loan status table
...
select loan_status.loan_id, loan_status.customer_id, loan_status.loan_amount, loan_status.loan_amount_term, 
loan_status.cibil_score, remarks_update.cibil_score_status from loan_status inner join remarks_update on
loan_status.loan_id = remarks_update.loan_id;

-- joining loan status and remarks update table

create table loan_cibil_score_details select loan_status.loan_id, loan_status.customer_id, loan_status.loan_amount, loan_status.loan_amount_term, 
loan_status.cibil_score, remarks_update.cibil_score_status from loan_status inner join remarks_update on
loan_status.loan_id = remarks_update.loan_id;
...
select loan_cibil_score_details.loan_id, loan_cibil_score_details.customer_id, loan_cibil_score_details.loan_amount, loan_cibil_score_details.loan_amount_term, 
loan_cibil_score_details.cibil_score, loan_cibil_score_details.cibil_score_status, customer_income.applicant_income, customer_income.coapplicant_income,
customer_income.Property_Area, customer_income.Loan_Status, customer_info.customer_name, customer_info.gender,
customer_info.age, customer_info.married, customer_info.education, customer_info.self_employed, customer_info.region_id,
country_state.postal_code, country_state.segment, country_state.state, region_info.region 
from loan_cibil_score_details inner join customer_income on loan_cibil_score_details.loan_id = customer_income.loan_id
inner join customer_info on loan_cibil_score_details.customer_id = customer_info.customer_id
inner join country_state on loan_cibil_score_details.loan_id = country_state.loan_id
inner join region_info on country_state.region_id = region_info.region_id;

-- joining all the 5 tables

create table Loan_details select loan_cibil_score_details.loan_id, loan_cibil_score_details.customer_id, loan_cibil_score_details.loan_amount, loan_cibil_score_details.loan_amount_term, 
loan_cibil_score_details.cibil_score, loan_cibil_score_details.cibil_score_status, customer_income.applicant_income, customer_income.coapplicant_income,
customer_income.Property_Area, customer_income.Loan_Status, customer_info.customer_name, customer_info.gender,
customer_info.age, customer_info.married, customer_info.education, customer_info.self_employed, customer_info.region_id,
country_state.postal_code, country_state.segment, country_state.state, region_info.region 
...
delete from customer_criteria where Loan_amount = 'Loan still processing';

alter table customer_criteria modify loan_amount int;

create table loan_det_cal select *, round((loan_amount * interest_rate/100),2) as Monthly_interest_amount, 
round((loan_amount * interest_rate/100 *12),2) as Annual_interest_amount from customer_criteria;

select * from loan_det_cal;

-- updating some details - for given criteria

update loan_det_cal set gender = "Female" where customer_id = 'IP43006';
update loan_det_cal set gender = "Female" where customer_id = 'IP43016';
update loan_det_cal set gender = "Male" where customer_id = 'IP43018';
update loan_det_cal set gender = "Male" where customer_id = 'IP43038';
update loan_det_cal set gender = "Female" where customer_id = 'IP43508';
update loan_det_cal set gender = "Female" where customer_id = 'IP43577';
update loan_det_cal set gender = "Female" where customer_id = 'IP43589';
update loan_det_cal set gender = "Female" where customer_id = 'IP43593';
update loan_det_cal set age = 45 where customer_id = 'IP43007';
update loan_det_cal set age = 32 where customer_id = 'IP43009';

delete from loan_det_cal where cibil_score <200;

select state, count(customer_id) as Customer_count, sum(loan_amount) as Total_loan_amount,
sum(applicant_income) as Total_applicant_income from loan_det_cal group by state;

select customer_name as customer_first_name, right(customer_name, 4) as customer_last_name from loan_det_cal where customer_name like '% ____';

-- store procedure : for final results

Delimiter $$
create procedure Final_result()
begin
select * from loan_Details;
...
select customer_name, right(customer_name, 4) as customer_last_name from loan_det_cal where customer_name like '% ____';
